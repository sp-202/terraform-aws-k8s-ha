...
resource "aws_launch_template" "k8s_worker_node" {
  ...
  user_data = base64encode(<<-BASH
              ...
              while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
                  if sudo kubeadm join ${aws_instance.master.public_ip}:6443 --token ${local.kubeadm_token} --discovery-token-unsafe-skip-ca-verification --node-name k8s-gp-node-$INSTANCE_ID; then
                      JOIN_SUCCESS=true
                      break
                  fi
