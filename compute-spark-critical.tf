...
resource "aws_launch_template" "worker_spark_critical" {
  ...
  user_data = base64encode(<<-BASH
              ...
              while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
                  if sudo kubeadm join ${aws_instance.master.public_ip}:6443 --token ${local.kubeadm_token} --discovery-token-unsafe-skip-ca-verification --node-name spark-node-$INSTANCE_ID; then
                      JOIN_SUCCESS=true
                      break
                  fi
