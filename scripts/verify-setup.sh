...
# Check Spark Optimization Sysctl
FILE_MAX=$(sysctl -n fs.file-max 2>/dev/null || echo "0")
if [ "$FILE_MAX" -eq 1000000 ]; then
    print_status 0 "Sysctl fs.file-max: 1000000 (Spark Optimized)"
else
    print_status 1 "Sysctl fs.file-max: $FILE_MAX (Expected 1000000)"
fi

VM_MAX_MAP_COUNT=$(sysctl -n vm.max_map_count 2>/dev/null || echo "0")
if [ "$VM_MAX_MAP_COUNT" -eq 262144 ]; then
    print_status 0 "Sysctl vm.max_map_count: 262144 (Sedona Optimized)"
else
    print_status 1 "Sysctl vm.max_map_count: $VM_MAX_MAP_COUNT (Expected 262144)"
fi

# Check Cgroup v2
if [ -d "/sys/fs/cgroup/cgroup.procs" ]; then
    print_status 0 "Cgroup v2: Detected"
else
    print_status 1 "Cgroup v2: Not detected"
fi

# Check NVMe Mount (if device exists)
if [ -b "/dev/nvme0n1" ]; then
    if mount | grep -q "/mnt/spark-nvme"; then
        print_status 0 "NVMe Mount: /mnt/spark-nvme (Mounted)"
    else
        print_status 1 "NVMe Mount: /mnt/spark-nvme (Not Mounted)"
    fi
else
    echo -e "  ${YELLOW}→${NC} NVMe device /dev/nvme0n1 not found (Skipping check)"
fi
